...
def fetch_data():
    ...
    try:
        ...
        # ✅ Rescale using CSV backup (if available)
        try:
            csv_data = pd.read_csv("tnpl_data.csv", index_col=0, parse_dates=True)
            if not csv_data.empty:
                ref_latest = csv_data['Close'].dropna().iloc[-1]
                alpha_latest = data['Close'].dropna().iloc[-1]
                factor = ref_latest / alpha_latest
                data['Close'] = data['Close'] * factor
                st.info("📡 Data Source: Alpha Vantage (Rescaled using CSV)")
            else:
    ...
    try:
        ...
        if not data.empty:
            ...
            st.error("❌ CSV exists but is empty.")
            st.stop()
    except Exception as e:
        st.error(f"❌ No data available at all. Error: {e}")
        st.stop()
